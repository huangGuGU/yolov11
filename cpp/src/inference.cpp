...
//}
//
//void Inference::get_img(const string &img_path) {
...
//}
//
//void Inference::get_img(const cv::Mat &mat) {
//    this->frame = mat.clone();
//    boxes.clear();
//    confidences.clear();
//}
//
//void Inference::infer_img() {
...
//}
//
//tuple<cv::Mat, vector<vector<float>>> Inference::nms() {
...
void Inference::infer_img() {
    ...
    int channel_num = outputs[0].size[1];
    for (int i = 0; i < rows; ++i) {
        float* classes_scores = &data[i * channel_num + 4];
        ...
        if (maxClassScore > conf_threshold) {
            ...
            float x = data[i * channel_num + 0];
            float y = data[i * channel_num + 1];
            float w = data[i * channel_num + 2];
            float h = data[i * channel_num + 3];
